...
Instructions:
1. Select 3-5 activities that fit today's weather using only the provided places.
2. For each activity, use the 'category' field to determine the type of activity:
...
4. Generate a short title and a 1-sentence description for each activity.
5. Provide **only ONE clothing advice for the day** based on the weather.

Respond in JSON with double quotes:
{{
    "activities": [
...
def weather_activity_agent(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        parsed = {"error": "Failed to parse LLM JSON output", "raw_response": response}
